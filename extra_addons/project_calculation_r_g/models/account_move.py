...
class AccountMove(models.Model):
    ...
    is_rg_discount_line = fields.Boolean(
        string='Is RG Discount Line',
        default=False,
        help="True if this line represents a retention guarantee discount"
    )
    ...
    def create(self, vals):
        # When invoice is created, check if it's related to a delivery with RG flag
        result = super(AccountMove, self).create(vals)

        # If invoice has an origin (usually sale order name)
        if result.invoice_origin:
            # Find all completed pickings with this origin
            ...
            if pickings:
                # Use the value from the most recent completed picking
                result.apply_rg_on_amount_total = pickings[0].apply_rg_on_amount_total
            else:
                # If no picking is done yet, check the sale order
                sale_order = self.env['sale.order'].search([
                    ('name', '=', result.invoice_origin)
                ], limit=1)

                if sale_order:
                    result.apply_rg_on_amount_total = sale_order.apply_rg_on_amount_total
        return result

    exclude_from_cumulative = fields.Boolean(
        string='Exclude from Cumulative',
        default=False,
        help="True if this line should be excluded from cumulative calculations"
    )
    ...
    original_amount_total = fields.Monetary(
        string='Original Amount Total',
        currency_field='currency_id',
        copy=False,
        readonly=True,
        help="Total amount before RG discount line"
    )
    ...
    def _compute_cumulative_totals(self):
        for move in self:
            ...
            move.cumulative_subtotal = sum(
            move.invoice_line_ids.filtered(lambda l: not l.is_rg_discount_line)
            .mapped('cumulative_price')
        )
    ...
    @api.depends('invoice_line_ids', 'amount_total', 'original_amount_total')
        ...
        for move in self:
            ...
            if sale_order and sale_order.analytic_account_id:
                ...
                if project and project.retenue_garantie:
                    ...
                    # Use original_amount_total for calculations if available
                    amount_for_calculation = move.original_amount_total if move.original_amount_total > 0 else move.amount_total
                    potential_rg = amount_for_calculation * (retention_percentage / 100.0)
                    ...
                    # Use original_amount_total for montant_rg_du calculation if available
                    move.montant_rg_du = amount_for_calculation - move.calculated_rg
                else:
                    move.retenue_garantie = 0.0
                    move.calculated_rg = 0.0
                    move.montant_rg_du = 0.0
    ...
    # update standar montant total
    def action_post(self):
        """Override the post action to add RG discount line before posting"""
        for move in self:
            print(move.apply_rg_on_amount_total)
            if (move.move_type in ('out_invoice', 'out_refund') and
                    not move.is_rg_invoice and
                    move.apply_rg_on_amount_total and
                    move.calculated_rg > 0):
                # Check if this invoice has an RG amount
                if move.calculated_rg > 0:
                    analytic_account = False
                    if move.invoice_line_ids:
                        # Try to find a line with analytic account
                        for line in move.invoice_line_ids:
                            if line.analytic_account_id and not line.is_rg_discount_line:
                                analytic_account = line.analytic_account_id
                                break

                    # Check if we already have an RG discount line
                    existing_rg_line = move.invoice_line_ids.filtered(lambda l: l.is_rg_discount_line)

                    if not existing_rg_line:
                        move.original_amount_total = move.amount_total
                        # Create discount product if it doesn't exist
                        rg_discount_product = self.env['product.product'].search(
                            [('default_code', '=', 'RG_DISCOUNT'), ('type', '=', 'service')],
                            limit=1
                        )

                        if not rg_discount_product:
                            rg_discount_product = self.env['product.product'].create({
                                'name': 'Remise retenue de garantie',
                                'default_code': 'RG_DISCOUNT',
                                'type': 'service',
                        ...
                        # Get 20% VAT tax
                        tax_20_percent = self.env['account.tax'].search([
                            ('type_tax_use', '=', 'sale'),
                            ('amount', '=', 20),
                            ('company_id', '=', move.company_id.id)
                        ], limit=1)

                        # If no 20% tax found, get any sales tax
                        if not tax_20_percent:
                            tax_20_percent = self.env['account.tax'].search([
                                ('type_tax_use', '=', 'sale'),
                                ('company_id', '=', move.company_id.id)
                            ], limit=1)

                        tax_ids = [(6, 0, [tax_20_percent.id])] if tax_20_percent else []

                        # Calculate the amount excluding tax (HT)
                        rg_amount_ht = move.calculated_rg / 1.2  # For 20% VAT

                        # Use the ORM to write the line to the invoice
                        move.write({
                            'invoice_line_ids': [(0, 0, {
                                'product_id': rg_discount_product.id,
                                'name': 'Remise retenue de garantie',
                                'quantity': 1.0,
                                'price_unit': -rg_amount_ht,  # Negative amount for discount
                                'tax_ids': tax_ids,
                                'is_rg_discount_line': True,
                                'exclude_from_cumulative': True,
                                'analytic_account_id': analytic_account.id if analytic_account else False,
                            })]
                        })

                        # Allow Odoo to handle the balancing
                        move._recompute_tax_lines()
                        move._onchange_invoice_line_ids()

        # Call the standard post method
