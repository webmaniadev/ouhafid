...
class SaleOrderLine(models.Model):
    ...
    pack_depth = fields.Integer(
        "Depth", help="Profondeur du produit s'il fait partie d'un pack."
    )
    pack_parent_line_id = fields.Many2one(
        "sale.order.line",
        "Pack",
        help="Le pack qui contient ce produit.",
    )
    pack_child_line_ids = fields.One2many(
        "sale.order.line", "pack_parent_line_id", "Lines in pack"
    )
    pack_modifiable = fields.Boolean(help="Le pack parent est modifiable")
