...
class PurchaseOrderLine(models.Model):
    ...
    def expand_pack_line(self, write=False):
        ...
        if self.product_id.pack_ok and self.pack_type == "detailed":
            for subline in self.product_id.get_pack_lines():
                ...
                if write:
                    # In Odoo 14, first() was not imported from odoo.fields
                    # Use filtered with first element instead
                    existing_subline = self.pack_child_line_ids.filtered(
                        lambda child: child.product_id == subline.product_id
                    )
                    existing_subline = existing_subline and existing_subline[0] or False
    ...
    def create(self, vals_list):
        # In Odoo 14, create method doesn't have model_create_multi by default
        ...
        for line_vals in vals_list:
            ...
            if product and product.pack_ok and product.pack_type != "non_detailed":
                line = super(PurchaseOrderLine, self).create(line_vals)
        ...
        if new_vals:
            res |= super(PurchaseOrderLine, self).create(new_vals)
        return res
    ...
    def action_open_parent_pack_product_view(self):
        ...
        return {
            ...
            "view_type": "form",  # In Odoo 14, view_type is still used
            "view_mode": "tree,form",
            "domain": domain,
        }
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        ...
        for line in self:
            ...
            # In Odoo 14, the params parameter is not used in _select_seller
