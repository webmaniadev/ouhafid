...
class ProductPackLine(models.Model):
    ...
    def get_seller_cost(self, line):
        ...
        self.ensure_one()
        if line:
            # In Odoo 14, params was not a standard parameter for _select_seller
            # So we need to use the direct parameters
            pack_line_seller = self.product_id._select_seller(
                partner_id=line.partner_id,
                quantity=self.quantity,
                date=line.order_id.date_order and line.order_id.date_order.date(),
                uom_id=line.product_uom,
                # Remove params parameter as it's not used in v14
            )
            return (
