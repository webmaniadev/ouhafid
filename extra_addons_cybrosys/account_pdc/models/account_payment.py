...
class AccountPayment(models.Model):
    ...
    def print_checks(self):
        ...
        if len(self) == 0:
            raise UserError(_("Payments to print as checks must have 'Check' or 'PDC' selected as payment method and "
                              "not be reconciled yet."))
    ...
    def _get_move_vals(self, journal=None):
        ...
        date = self.effective_date if self.payment_method_code == 'pdc' else self.payment_date
        return {
            'name': name,
            'date': date,
